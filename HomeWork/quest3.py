...
def ask_questions(questions):
    ...
    for i, q in enumerate(questions):
        user_answer = input(f"Question {i + 1}: {q['question']} ").strip().lower()
...
def main():
    ...
    print(f"{user_name}, your score is {score} out of {len(questions)}")

    result_file_format = input("Enter the result file format to save (CSV or JSON): ").strip().lower()
    if result_file_format == 'json':
        save_result_to_json('results.json', user_name, score)
    elif result_file_format == 'csv':
